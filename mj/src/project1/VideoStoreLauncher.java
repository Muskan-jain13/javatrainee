...
class VideoStore {
	...
	void addVideo(String videoName) {
		...
		VideoStoreLauncher.l[VideoStoreLauncher.no] = v;
	...
	void listInventory() {
		if (VideoStoreLauncher.no == 0) {
			...
			for (int i = 0; i < VideoStoreLauncher.no; i++) {
				System.out.println("Name " + VideoStoreLauncher.l[i].title + " Rating " + VideoStoreLauncher.l[i].rating
						+ " which is checked " + VideoStoreLauncher.l[i].isChecked);
			}
		}
	}
...
//				code of adding video
			...
			if (inputed == 3) {
//				checkout code
				System.out.println("enter the name of video to be checked out :");
				enter.nextLine();
				name = enter.nextLine();
				TSERIES.checkOut(name);
			}
			if (inputed == 4) {
				System.out.println("enter the name of video to be return out :");
				enter.nextLine();
				name = enter.nextLine();
